...
export default function ReporteAsistenciaClientes() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 3. Cargar Asistencia de Citas de la Agenda
        const { data: snapAgenda } = await supabase.from("agenda").select("*").or(`tenant_id.eq.${userProfile.inquilino},inquilino.eq.${userProfile.inquilino}`);
        const listData = [];
        ...
        (snapAgenda || []).forEach(c => {
          ...
          const est = (c.estado || "Atendida").toUpperCase();
          const asistioOk = est === "ATENDIDA" || est === "COMPLETADA" || est === "EN CONSULTA";

          listData.push({
            id: c.id,
            fechaObj: dateObj,
            fechaCitaStr: isNaN(dateObj.getTime()) ? (c.fecha || "") : format(dateObj, "dd/MM/yyyy HH:mm"),
            profesional: c.odontologo || c.profesional || c.doctor || "—",
            sucursal: c.sucursal || c.oficina || listSuc[0]?.nombre || "ATM CENTRO DEL DOLOR OROFACIAL",
            tipoDocPaciente: c.tipoDocPaciente || "CC",
            numDocPaciente: c.pacienteDocumento || c.documento || "—",
            nombrePaciente: c.pacienteNombre || c.paciente || "—",
            estadoCita: c.estado || "Atendida",
            asistio: asistioOk ? "SI" : "NO"
          });
        });

        listData.sort((a, b) => b.fechaObj - a.fechaObj);
        setAsistenciaList(listData);
        filterData(listData, appliedFilters, "");

      } catch (error) {
        console.error("Error cargando asistencia de clientes:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const filterData = (sourceList, filters, quickSearch) => {
    let result = sourceList.filter(item => {
      ...
      if (filters.fechaInicial && filters.fechaFinal) {
        ...
        if (item.fechaObj < init || item.fechaObj > end) return false;
      }
      ...
      if (filters.profesional && filters.profesional !== "TODOS") {
        ...
        const itemProf = (item.profesional || "").toLowerCase();
        if (!itemProf.includes(targetProf) && !targetProf.includes(itemProf)) return false;
      }
    ...
    if (quickSearch && quickSearch.trim() !== "") {
      ...
      result = result.filter(item => (
        item.nombrePaciente.toLowerCase().includes(term) ||
        item.numDocPaciente.toLowerCase().includes(term) ||
        item.profesional.toLowerCase().includes(term)
      ));
    }

    setFilteredAsistencia(result);
  };
  ...
  const handleSearchClick = () => {
    ...
    filterData(asistenciaList, newFilters, tableSearchTerm);
  };

  const handleExportExcel = () => {
    const rows = filteredAsistencia.map(item => ({
      "Fecha y hora cita": item.fechaCitaStr,
      "Profesional": item.profesional,
      "Sucursal": item.sucursal,
      "T. Doc. Paciente": item.tipoDocPaciente,
      "Num. Doc. Paciente": item.numDocPaciente,
      "Nombre paciente": item.nombrePaciente,
      "Estado cita": item.estadoCita,
      "Asistencia confirmada": item.asistio
    }));

    const worksheet = XLSX.utils.json_to_sheet(rows);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "AsistenciaClientes");
    XLSX.writeFile(workbook, `Asistencia_Clientes_${appliedFilters.fechaInicial}_al_${appliedFilters.fechaFinal}.xlsx`);
  };
    ...
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden font-sans text-slate-700">
      ...
      <div className="flex items-center justify-between px-5 py-3 bg-white border-b border-slate-100 shrink-0">
        <div className="flex items-center gap-2">
          <h2 className="text-base font-black text-slate-800 tracking-tight">Asistencia de clientes</h2>
          <div className="flex items-center gap-1.5 text-[11px] text-slate-400 font-medium ml-2">
            <span>🏠 Reportes</span>
            <span>/</span>
            <span className="text-slate-500 font-bold">Asistencia de clientes</span>
          </div>
        </div>
      ...
      {/* ─── ÁREA DE FILTROS 1:1 CON LOS DEMÁS REPORTES ─── */}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    filterData(asistenciaList, appliedFilters, e.target.value);
                  }}
                    ...
                    <tr key={item.id} className="hover:bg-sky-50/40 transition-colors">
                      ...
                      {visibleColumns.asistio && (
                        <td className="px-3 py-2 text-center whitespace-nowrap">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase ${item.asistio === "SI" ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}>
                            {item.asistio}
                          </span>
                        </td>
                      )}
