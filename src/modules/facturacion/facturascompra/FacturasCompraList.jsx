...
import { FiTruck, FiSearch, FiCalendar, FiPrinter, FiEye } from "react-icons/fi";
...
export default function FacturasCompraList({ onNew }) {
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const q = query(collection(db, "facturas_compra"), where("inquilino", "==", inquilino));
      ...
      const list = snap.docs.map(d => ({ id: d.id, ...d.data() }))
        .filter(f => {
          ...
          const ts = f.fecha.toDate ? f.fecha.toDate().getTime() : new Date(f.fecha).getTime();
  ...
  const filtered = facturas.filter(f =>
    ...
    (f.nroFactura || f.id || "").toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Proveedor o # Factura..." className="w-full h-11 pl-12 pr-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-600 outline-none focus:border-blue-500 transition-all"
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 bg-slate-50 z-10 border-b border-slate-100">
              <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                <th className="px-6 py-4 pl-8"># Factura</th>
                ...
                <tr>
                  <td colSpan="8" className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Cargando facturas...</span>
                    </div>
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                  ...
                  <td colSpan="8" className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-3">
                      <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center">
                        <FiTruck className="text-slate-300" size={28} />
                      </div>
                      <p className="text-xs font-black text-slate-400 uppercase tracking-widest">No se encontraron facturas de compra</p>
                      <p className="text-[11px] text-slate-400">Registra las facturas que recibes de tus proveedores</p>
                    </div>
                  </td>
                </tr>
              ) : filtered.map(f => (
                <tr key={f.id} className="hover:bg-slate-50/30 transition-colors group">
                  <td className="px-6 py-4 pl-8 font-bold font-mono text-slate-800">#{f.nroFactura || f.id.slice(0,8).toUpperCase()}</td>
                  <td className="px-6 py-4 font-semibold text-slate-500">{fmtDate(f.fecha)}</td>
                  <td className="px-6 py-4 font-bold text-slate-800 uppercase">{f.proveedor || "—"}</td>
                  <td className="px-6 py-4 text-slate-600">{f.concepto || "—"}</td>
                  <td className="px-6 py-4 text-right font-semibold text-slate-600 font-mono">{fmt(f.subtotal)}</td>
                  <td className="px-6 py-4 text-right font-semibold text-slate-500 font-mono">{fmt(f.iva)}</td>
                  <td className="px-6 py-4 text-right font-black text-slate-900 font-mono">{fmt(f.total)}</td>
